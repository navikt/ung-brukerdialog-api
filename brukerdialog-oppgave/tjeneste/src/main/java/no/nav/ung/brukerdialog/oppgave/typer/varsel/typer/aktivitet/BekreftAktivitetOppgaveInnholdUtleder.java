...
@ApplicationScoped
public class BekreftAktivitetOppgaveInnholdUtleder implements OppgaveInnholdUtleder {
    ...
    BekreftAktivitetOppgaveInnholdUtleder() {
        // for CDI proxy
    }

    @Inject
    public BekreftAktivitetOppgaveInnholdUtleder(
    ...
    private OppgaveTekstfragmentRenderer.Resultat rendre(BrukerdialogOppgaveEntitet oppgave) {
        ...
        LocalDate fom;
        LocalDate tom;
        AktivitetsvilkåretIkkeOppfyltÅrsak årsak;
        String fritekst;
        AktivitetsavklaringKildeType kilde;
        String kildeFritekst;

        switch (dto) {
            case BekreftAktivitetOppgavetypeDataDto bundet -> {
                fom = bundet.fom();
                tom = bundet.tom();
            ...
            case BekreftAktivitetOpphørOppgavetypeDataDto opphør -> {
                fom = opphør.fom();
                tom = null;
            ...
            default -> throw new IllegalArgumentException(
                "Ikke støttet oppgavedata for " + OppgaveType.BEKREFT_AKTIVITET + ": " + dto.getClass().getName());
        }

        return renderer.rendre("tekstfragmenter/aktivitet/bekreft_aktivitet",
            byggData(årsak, fom, tom, fritekst, kilde, kildeFritekst, oppgave.getFristTid()));
    }

    private static Map<String, Object> byggData(
        AktivitetsvilkåretIkkeOppfyltÅrsak årsak, LocalDate fom, LocalDate tom, String fritekst,
        AktivitetsavklaringKildeType kilde, String kildeFritekst, LocalDateTime fristTid
    ) {
    ...
    private static void validerYtelsetype(OppgaveYtelsetype ytelsetype) {
        if (ytelsetype != OppgaveYtelsetype.AKTIVITETSPENGER) {
            throw new IllegalStateException(
                "BEKREFT_AKTIVITET støtter kun AKTIVITETSPENGER, fikk ytelsetype=%s".formatted(ytelsetype));
        }
    }
}
